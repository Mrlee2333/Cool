...
const BLOCKED_EXTENSIONS_RE = /\.(jpe?g|png|gif|webp|vtt)$/i;
...
const whitelistStrategies = [
  {
    ...
    // 这里已经使用了我们讨论的最佳实践 .some() 方法
    detector: (url) => {
      const targetDomains = ['kkzycdn.com', 'ryplay17.com', '360zyx.vip'];
...
function isUniversalAd(url) {
    ...
    if (BLOCKED_EXTENSIONS_RE.test(lowerUrl)) {
        console.log(`[黑名单] 命中: 非法文件扩展名 -> ${url}`);
        return true;
    }
